...
class Modelo:
    ...
    def __init__(self, origen, df, aviones, jornada, entrevistadores, descanso, velocidad, ocupacion, exito, f_o, solver):
        ...
        self.__descanso_min = datetime.strptime(str(self.__descanso), '%M').time()
        self.__jornada_min = datetime.strptime(str(self.__jornada), '%H').time()
    ...
    def calculos_iniciales(self):
        ...
        for i in range(len(self.__df_aviones)):
            ...
            ocupacion = ceil(asientos * self.__ocupacion); encuestas = ceil(ocupacion * self.__exito)
            auxConsumo = ("{:%H:%M:%S}".format(datetime.min + timedelta(minutes= encuestas * self.__velocidad)))
            consumo = datetime.strptime(auxConsumo, '%H:%M:%S').time()

            self.__df.loc[(self.__df.aeronave == modelo), 'num_asientos'] = asientos
            self.__df.loc[(self.__df.aeronave == modelo), 'ocupacion'] = ocupacion
            self.__df.loc[(self.__df.aeronave == modelo), 'total_encuestas'] = encuestas
    ...
    def formatos_horas(self):
        ...
        for hora in horas:
            self.__df.loc[(self.__df.hora_salida == hora), 'hora_salida'] = datetime.strptime(hora, '%H:%M').time()

        # Se cambia el formato del consumo
        consumos = (self.__df['consumo']).unique()
        for consumo in consumos:
            self.__df.loc[(self.__df.consumo == consumo), 'consumo'] = datetime.strptime(str(consumo), '%H:%M:%S').time()
    ...
    def calculos_destinos(self):
        ...
        self.__i_p = [(self.__df.index[self.__df.pais == self.__destinos[p]]).tolist() for p in self.__P]
    ...
    def resolver(self):
        ...
        for i in self.__I:
            for k in self.__K:
                self.__x[i,k] = LpVariable('x(' + str(i) + ',' + str(k) + ')', 0, 1, LpBinary)
        ...
        for i in self.__I:
            for j in self.__I:
                ...
                and self.__df['hora_salida'][i] <= self.__df['hora_salida'][j]):
                    for k in self.__K:
                        
                        hora_i = datetime.combine(
                            date.min, self.__df['hora_salida'][i])- datetime.min
                        
                        hora_j = datetime.combine(
                            date.min, self.__df['hora_salida'][j]) - datetime.min
                        
                        consumo_i = datetime.combine(
                            date.min, self.__df['consumo'][i]) - datetime.min
                        
                        consumo_j = datetime.combine(
                            date.min, self.__df['consumo'][j]) - datetime.min
                        
                        desc = datetime.combine(
                            date.min, self.__descanso_min) - datetime.min
                        
                        jorn = datetime.combine(
                            date.min, self.__jornada_min) - datetime.min
                        
                        if (hora_i > hora_j - (consumo_j / 2) - desc
                            ...
                            solver += self.__x[i,k] + self.__x[j,k] <= 1 + lpSum(self.__x[j,l] for l in self.__K if l != k)
                        elif hora_i - consumo_i < hora_j - jorn:
                        #elif hora_i - consumo_i + jorn < hora_j:
                        #elif hora_i - consumo_i - desc + jorn < hora_j:
                            solver += self.__x[i,k] + self.__x[j,k] <= 1 + lpSum(self.__x[i,l] for l in self.__K if l != k)
    ...
    def formatear_solucion(self):
        ...
        for p in self.__P:
            encuestas_1 = 0; encuestas_2 = 0; vuelos_encuestador_1 = 0; vuelos_encuestador_2 = 0
            regiones.append(self.__destinos[p])
            origen.append(self.__origen)
            for i in self.__i_p[p]:
                if self.__y1[i].varValue <= 1.1 and self.__y1[i].varValue >= 0.9:
                    vuelos_encuestador_1 += 1; encuestas_1 += self.__df['total_encuestas'][i]
                elif self.__y2[i].varValue <= 1.1 and self.__y2[i].varValue >= 0.9:
                    vuelos_encuestador_2 += 1; encuestas_2 += self.__df['total_encuestas'][i]
            
            l_encuestas_1.append(encuestas_1); l_encuestas_2.append(encuestas_2); pasajeros_encuestados.append(encuestas_1+encuestas_2)  
        ...
        self.__df_solucion = pd.concat([self.__df_solucion ,pd.DataFrame({"origen":  origen,"regiones": regiones, "vuelos_total": num_vuelos_total, "pasajeros_total": pasajeros_total, "encuestas_min": encuestas_minimas, "encuestas_max": encuestas_maximas, "encuestas_1_encuestador": l_encuestas_1, "encuestas_2_encuestadores": l_encuestas_2, "total_encuestas": pasajeros_encuestados, "donaciones": donaciones, "vuelos_encuestados_1_encuestador": l_vuelos_encuestador_1, "vuelos_encuestados_2_encuestadores": l_vuelos_encuestador_2, "total_vuelos_encuestados": num_vuelos_encuestados})])
